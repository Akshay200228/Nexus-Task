...
const Nav = () => {
    ...
    return (
        <motion.header
            className={`fixed top-0 z-50 w-full py-8 padding-x ${scrolled ? 'bg-white' : 'bg-transparent'}`}
                ...
                <ul className='flex items-center justify-center flex-1 gap-16 max-lg:hidden'>
                    {
                        navLinks.map(({ label, href }) => (
                            <li key={label}>
                                <a
                                    href={href}
                                    className={`p-2 text-lg leading-normal transition duration-300 rounded-lg font-montserrat hover:bg-coral-red hover:text-white ${scrolled ? 'hover:text-white' : 'hover:text-slate-gray'}`}
                                >
                                    {label}
                                </a>
                            </li>
